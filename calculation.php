...
//$url='https://api.mysportsfeeds.com/v2.1/pull/nfl/2019-2020-regular/games.json'; 
...
$data=json_decode($resp,true);
 ...
 echo "<tr>";
 echo '<td>Team 1: <select onchange1="setteam(this.value,1)" id="teams1g">';
 for($i  = 0;$i < count($data['games']);$i++){
		?>
			<option value="<?php  echo number_format($data['games'][$i]['score']['homeScoreTotal']*team_algorithm_team1_win_home(),1);?>,<?php  echo number_format($data['games'][$i]['score']['awayScoreTotal']*team_algorithm_team1_win_away(),1);?>,<?php  echo $data['games'][$i]['schedule']['homeTeam']['abbreviation'];?>"> <?php  echo $data['games'][$i]['schedule']['homeTeam']['abbreviation'];?></option>
		
		<?php
 }
 echo '</select></td>';
 echo '<td> Team 2: <select  onchange1="setteam(this.value,2)" id="teams2g">';
 for($i  = 0;$i < count($data['games']);$i++){
		?>
			<option value="<?php  echo number_format($data['games'][$i]['score']['homeScoreTotal']*team_algorithm_team2_win_home(),1);?> , <?php  echo number_format($data['games'][$i]['score']['awayScoreTotal']*team_algorithm_team2_win_away(),1);?>,<?php  echo $data['games'][$i]['schedule']['homeTeam']['abbreviation'];?>"> <?php  echo $data['games'][$i]['schedule']['homeTeam']['abbreviation'];?></option>
		
		<?php
 }
 echo '</select> <input type="button" name="Submit" value="Submit" onclick="setteam()"></td>';
 echo '</tr></Table>';
 die;
echo '<Table style="width: 100%;">
